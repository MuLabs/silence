    ...
    class HTML2PDF
    {
        ...
        protected function _vueHTML($content)
        {
            $content = preg_replace(
                '/<page_header([^>]*)>/isU',
                '<hr>' . HTML2PDF_locale::get('vue01') . ' : $1<hr><div$1>',
                $content
            );
            $content = preg_replace(
                '/<page_footer([^>]*)>/isU',
                '<hr>' . HTML2PDF_locale::get('vue02') . ' : $1<hr><div$1>',
                $content
            );
            $content = preg_replace(
                '/<page([^>]*)>/isU',
                '<hr>' . HTML2PDF_locale::get('vue03') . ' : $1<hr><div$1>',
                $content
            );
...
' . $content . '
                ...
                if (is_array($this->_background)) {
                    ...
                    if (isset($this->_background['img']) && $this->_background['img'])
                        $this->pdf->Image(
                            ...
                            $this->_background['posX'],
                            $this->_background['posY'],
        ...
        protected function _setMargins()
        {
            ...
            if ($this->_isInParagraph !== false) {
                $this->_pageMarges[floor($this->_margeTop * 100)] = array(
                    $this->_isInParagraph[0],
                    $this->pdf->getW() - $this->_isInParagraph[1]
                );
            } else {
                $this->_pageMarges[floor($this->_margeTop * 100)] = array(
                    $this->_margeLeft,
                    $this->pdf->getW() - $this->_margeRight
                );
            }
        }
        ...
        protected function _DEBUG_add($name, $level = null)
        {
            ...
            $name = str_repeat(
                    '  ',
                    $this->_debugLevel
                ) . $name . ($level === true ? ' Begin' : ($level === false ? ' End' : ''));
            $time = microtime(true);
        ...
        protected function _executeAction($action)
        {
            ...
            if (!is_callable(array(&$this, $fnc))) {
                throw new HTML2PDF_exception(1, strtoupper($action['name']), $this->parsingHtml->getHtmlErrorCode(
                    $action['html_pos']
                ));
            }
        ...
        {
            if ($h) {
                if (($this->pdf->getY() + $h < $this->pdf->getH() - $this->pdf->getbMargin(
                        )) || $this->_isInOverflow || $this->_isInFooter
                )
        ...
        protected function _prepareTransform($transform)
        {
            ...
            for ($k = 0; $k < count($match[0]); $k++) {
                ...
                switch ($name) {
                    ...
                    case 'translate':
                        if (!isset($val[0])) $val[0] = 0.; else $val[0] = $this->parsingCss->ConvertToMM(
                            $val[0],
                            $this->_isInDraw['w']
                        );
                        if (!isset($val[1])) $val[1] = 0.; else $val[1] = $this->parsingCss->ConvertToMM(
                            $val[1],
                            $this->_isInDraw['h']
                        );
                    ...
                    case 'rotate':
                        ...
                        if (!isset($val[2])) $val[2] = 0.; else $val[2] = $this->parsingCss->ConvertToMM(
                            $val[2],
                            $this->_isInDraw['h']
                        );
                    ...
                    case 'matrix':
                        ...
                        if (!isset($val[4])) $val[4] = 0.; else $val[4] = $this->parsingCss->ConvertToMM(
                            $val[4],
                            $this->_isInDraw['w']
                        );
                        if (!isset($val[5])) $val[5] = 0.; else $val[5] = $this->parsingCss->ConvertToMM(
                            $val[5],
                            $this->_isInDraw['h']
                        );
                        $actions[] = $val;
        ...
        protected function _tag_open_PAGE($param)
        {
            ...
            if ($newPageSet) {
                ...
                if (isset($param['backimg'])) {
                    ...
                    if ($background['img']) {
                        ...
                        if (count($infos) > 1) {
                            ...
                            $background['posX'] = $this->parsingCss->ConvertToMM(
                                $background['posX'],
                                $this->pdf->getW() - $imageWidth
                            );
                            $background['posY'] = $this->parsingCss->ConvertToMM(
                                $background['posY'],
                                $this->pdf->getH() - $imageHeight
                            );
                        } else {
                            $background = array();
                        }
                ...
                $res = false;
                $background['color'] = isset($param['backcolor']) ? $this->parsingCss->convertToColor(
                    $param['backcolor'],
                    $res
                ) : null;
        ...
        protected function _tag_open_DIV($param, $other = 'div')
        {
            ...
            if (!$this->parsingCss->value['position']) {
                ...
                $old = $this->parsingCss->getOldValues();
                $parentWidth = $old['width'] ? $old['width'] : $this->pdf->getW() - $this->pdf->getlMargin(
                    ) - $this->pdf->getrMargin();
        ...
        protected function _tag_open_WRITE($param)
        {
            ...
            if (in_array(
                $this->parsingCss->value['id_tag'],
                array('fieldset', 'legend', 'div', 'table', 'tr', 'td', 'th')
            )
            ) {
                $fill = false;
            }
            ...
            if (count($words)) {
                ...
                $this->pdf->setXY($this->pdf->getX(), $y + $dh + $dy);
                $this->pdf->Cell(
                    ($align == 'L' ? $w : $this->parsingCss->value['width']),
                    $h,
                    $txt,
                    0,
                    0,
        ...
        protected function _tag_open_P($param)
        {
            ...
            $this->pdf->setXY(
                $this->pdf->getX() - $this->parsingCss->value['margin']['l'],
                $this->pdf->getY() - $this->parsingCss->value['margin']['t']
            );
            ...
            if ($this->parsingCss->value['text-indent'] > 0) {
                $y = $this->pdf->getY(
                    ) + $this->parsingCss->value['margin']['t'] + $this->parsingCss->value['padding']['t'];
                $this->_pageMarges[floor($y * 100)] = array(
                    $mL + $this->parsingCss->value['text-indent'],
                    $this->pdf->getW() - $mR
                );
        ...
        protected function _tag_open_UL($param, $other = 'ul')
        {
            ...
            $this->_listeAddLevel(
                $other,
                $this->parsingCss->value['list-style-type'],
                $this->parsingCss->value['list-style-image']
            );
        ...
        protected function _tag_open_TABLE($param, $other = 'table')
        {
            ...
            if ($this->_subPart) {
                ...
                HTML2PDF::$_tables[$param['num']]['border'] = isset($param['border']) ? $this->parsingCss->readBorder(
                    $param['border']
                ) : null;
                HTML2PDF::$_tables[$param['num']]['cellpadding'] = $this->parsingCss->ConvertToMM(
                    isset($param['cellpadding']) ? $param['cellpadding'] : '1px'
                );
                HTML2PDF::$_tables[$param['num']]['cellspacing'] = $this->parsingCss->ConvertToMM(
                    isset($param['cellspacing']) ? $param['cellspacing'] : '2px'
                );
        ...
        protected function _tag_close_TABLE($param)
        {
            ...
            if ($this->_subPart) {
                ...
                $this->_calculateTableCellSize(
                    HTML2PDF::$_tables[$param['num']]['cases'],
                ...
                if (count(HTML2PDF::$_tables[$param['num']]['height']) > 1)
                    $y = $this->_margeTop + HTML2PDF::$_tables[$param['num']]['height'][count(
                            HTML2PDF::$_tables[$param['num']]['height']
                        ) - 1];
                else if (count(HTML2PDF::$_tables[$param['num']]['height']) == 1)
                    $y = HTML2PDF::$_tables[$param['num']]['curr_y'] + HTML2PDF::$_tables[$param['num']]['height'][count(
                            HTML2PDF::$_tables[$param['num']]['height']
                        ) - 1];
                else
                    $y = HTML2PDF::$_tables[$param['num']]['curr_y'];
        ...
        protected function _tag_open_TR($param, $other = 'tr')
        {
            ...
            if (!$this->_subPart) {
                ...
                $ty = null;
                for ($ii = 0; $ii < count(
                    HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1]
                ); $ii++) {
                    $ty = max(
                        $ty,
                        HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][$ii]['h']
                    );
                }
                ...
                if (!$this->_isInTfoot && HTML2PDF::$_tables[$param['num']]['td_y'] + HTML2PDF::$_tables[$param['num']]['marge']['b'] + $ty + $hfoot > $this->pdf->getH(
                    ) - $this->pdf->getbMargin()
                ) {
                ...
                if (HTML2PDF::$_tables[$param['num']]['new_page'] && count(
                        HTML2PDF::$_tables[$param['num']]['thead']['code']
                    )
                ) {
                    ...
                    if (count($colParam['style']['width'])) {
                        ...
                        foreach ($colParam['style']['width'] as $width) {
                            if (substr($total, -1) == '%' && substr($width, -1) == '%')
                                ...
                                $total = ($this->parsingCss->ConvertToMM(
                                            $total,
                                            $last
                                        ) + $this->parsingCss->ConvertToMM($width, $last)) . 'mm';
                        }
                    }
            ...
            if ($this->_subPart) {
                ...
                for ($j = 0; $j < $rowspan; $j++) {
                    for ($i = 0; $i < $colspan; $i++) {
                        ...
                        [HTML2PDF::$_tables[$param['num']]['corr_x'] + $i] = ($i + $j > 0) ? '' : array(
                            $x,
                            $y,
                            $colspan,
                            $rowspan
        ...
        protected function _tag_close_TD($param)
        {
            ...
            if ($this->_subPart) {
                ...
                HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['w'] = max(
                    array($w0, $w2)
                );
                HTML2PDF::$_tables[$param['num']]['cases'][HTML2PDF::$_tables[$param['num']]['tr_curr'] - 1][HTML2PDF::$_tables[$param['num']]['td_curr'] - 1]['h'] = max(
                    array($h0, $h2)
                );
        ...
        public function setDefaultMargins($left, $top, $right = null, $bottom = null)
        {
            $this->_setDefaultMargins($left, $top, $right, $bottom);
        }
