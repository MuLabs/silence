...
class HTML2PDF_parsingCss
{
    ...
    public function getSvgStyle($tagName, &$param)
    {
        ...
        if (isset($styles['stroke-width'])) {
            $this->value['svg']['stroke-width'] = $this->convertToMM(
                $styles['stroke-width']
            );
        }
    ...
    public function analyse($tagName, &$param, $legacy = null)
    {
        ...
        if ($noWidth && in_array(
            ...
            if ($correctWidth) {
                ...
                if (in_array($tagName, array('th', 'td'))) {
                    $this->value['width'] -= $this->convertToMM(
        ...
        if ($this->value['top'] != null) {
            $this->value['top'] = $this->convertToMM(
                $this->value['top'],
                $this->getLastHeight(true)
            );
        }
        if ($this->value['bottom'] != null) {
            $this->value['bottom'] = $this->convertToMM(
                $this->value['bottom'],
                $this->getLastHeight(true)
            );
        }
        if ($this->value['left'] != null) {
            $this->value['left'] = $this->convertToMM(
                $this->value['left'],
                $this->getLastWidth(true)
            );
        }
        if ($this->value['right'] != null) {
            $this->value['right'] = $this->convertToMM(
                $this->value['right'],
                $this->getLastWidth(true)
            );
        }
    ...
    public function readBorder($css)
    {
        ...
        foreach ($css as $value) {
            ...
            if ($tmp !== null) {
                ...
                if (in_array($value, array('solid', 'dotted', 'dashed', 'double'))) {
                    $type = $value;
                ...
                if ($css[0] == 'right') {
                    ...
                    if ($css[0] == 'top') {
                        ...
                        if ($css[0] == 'bottom') {
                            ...
                            if (preg_match('/^[-]?[0-9\.]+%$/isU', $css[0])) {
                                $x = $css[0];
                            } else {
                                if ($this->convertToMM($css[0])) {
                ...
                if ($css[1] == 'top') {
                    ...
                    if ($css[1] == 'center') {
                        ...
                        if ($css[1] == 'bottom') {
                            ...
                            if (preg_match('/^[-]?[0-9\.]+%$/isU', $css[1])) {
                                $y = $css[1];
                            } else {
                                if ($this->convertToMM($css[1])) {
    ...
    public function convertToMM($css, $old = 0.)
    {
        ...
        if (preg_match('/^[0-9\.\-]+px$/isU', $css)) {
            ...
            if (preg_match('/^[0-9\.\-]+pt$/isU', $css)) {
                $css = 25.4 / 72. * str_replace('pt', '', $css);
            } else {
                if (preg_match('/^[0-9\.\-]+in$/isU', $css)) {
                    $css = 25.4 * str_replace('in', '', $css);
                } else {
                    if (preg_match('/^[0-9\.\-]+mm$/isU', $css)) {
    ...
    public function convertToColor($css, &$res)
    {
        ...
        if (preg_match(
            '/cmyk\([\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*\)/isU',
            $css,
            $match
        )
        ) {
